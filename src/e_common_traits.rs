...
impl PartialOrd for Employee {
	fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
		match self.uid.partial_cmp(&other.uid) {
...
impl TryFrom<String> for Employee {
	...
	fn try_from(value: String) -> Result<Self, Self::Error> {
		...
		let experience = parts[1].parse::<u32>().unwrap();
		let wage = parts[2].parse::<u32>().unwrap();
		let uid = parts[3].parse::<u32>().unwrap();

		if experience == u32::MAX || wage == u32::MAX || uid == u32::MAX {
...
impl From<Employee> for String {
	fn from(e: Employee) -> Self {
		let formated_string = format!("{}, {}, {}, {}", e.name, e.experience, e.wage, e.uid);
		formated_string
		// todo!()
	}
}
